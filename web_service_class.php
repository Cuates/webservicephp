<?php
  /*
          File: web_service_class.php
       Created: 07/23/2020
       Updated: 08/03/2020
    Programmer: Cuates
    Updated By: Cuates
       Purpose: Class for all Web Service interactions
  */

  // Include configuration file
  include ("web_service_config.php");
  ...
  class web_service_class extends web_service_config
  {
    ...
    public function web_service_class()
    {
    ...
    // Allocate web service call
    public function allocateWebService($param01, $param02)
    {
      // Return JSON string from API call
      return $this->allocateWSData($param01, $param02, '<Web_Service_Name>');
    }

    // Perform the allocate web service call
    private function allocateWSData($param01, $param02, $type = '<Web_Service_Name>')
    {
      ...
      try
      {
        ...
        {
          // Setup the url string with the query string intended for this call
          $urlQueryString = "";
          $urlQueryString .= $this->URL;
          $urlQueryString .= $this->URLAPI;
          $urlQueryString .= "path/allocate";

          // Setup the CURL call
          // Initialize the curl call
          $ch = curl_init();

          // Setup a payload in JSON for the server-side to utilize
          $payload = array("param01" => $param01, "param02" => $param02);
          $jsonPayload = json_encode($payload, true);
      ...
      catch(Exception $e)
      {
        ...
        // Set message
        $returnValue = 'SError~Caught retrieve allocate web service Execution Failure - ' . $e->getMessage();
    ...
    // Trigger web service call
    public function triggerWebService($param01)
    {
      // Return JSON string from API call
      return $this->triggerWSData($param01, '<Web_Service_Name>');
    }

    // Perform the trigger web service call
    private function triggerWSData($param01, $type = '<Web_Service_Name>')
    {
      // Initialize parameter
      $returnValue = "";
      ...
      catch(Exception $e)
      {
        ...
        // Set message
        $returnValue = 'SError~Caught retrieve trigger web service Execution Failure - ' . $e->getMessage();
    ...
    public function checkWebService($param01, $param02)
    {
      // Return JSON string from API call
      return $this->checkWSData($param01, $param02, '<Web_Service_Name>');
    }

    // Perform the check web service call
    private function checkWSData($param01, $param02, $type = '<Web_Service_Name>')
    {
      // Initialize parameter
      $returnValue = "";
      ...
      try
      {
        ...
        if (!isset($connectionStatus['SError']) && !array_key_exists('SError', $connectionStatus))
        {
          ...
          $urlQueryString .= $this->URL;
          $urlQueryString .= $this->URLAPI;
          $urlQueryString .= "path/check/param01/" . $param01 . "/param02/" . $param02;
      ...
      catch(Exception $e)
      {
        ...
        // Set message
        $returnValue = 'SError~Caught retrieve check web service Execution Failure - ' . $e->getMessage();
