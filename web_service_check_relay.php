<?php
  /*
          File: web_service_check_relay.php
       Created: 07/23/2020
       Updated: 07/26/2020
    Programmer: Cuates
    Updated By: Cuates
       Purpose: Check web service Relay
  */
  ...
  $scriptName = 'Web Service Check Relay'; // Production
  // $scriptName = 'TEST Web Service Check Relay TEST'; // Development
  ...
  try
  {
    ...
    // $errorPrefixFilename = "web_service_check_relay_issue_"; // Production
    $errorPrefixFilename = "web_service_check_relay_dev_issue_"; // Development
      ...
      if (is_array($webserviceResponse))
      {
        ...
        if(!isset($webserviceResponse['SError']) && !array_key_exists('SError', $webserviceResponse))
        {
          ...
          if ($server_response === "Success")
          {
            ...
            $value02Value = isset($webserviceResponse['Payload']['value02']) ? trim($webserviceResponse['Payload']['value02']) : trim("");
            $value03Value = isset($webserviceResponse['Payload']['value03']) ? trim($webserviceResponse['Payload']['value03']) : trim("no");

            // Check if variable is set
            if (($value01Value !== "") && ($value02Value !== ""))
            {
              // Extract new router data
              $extractDataResponse = $web_service_cl->checkWebService($value01Value, $value02Value);
              ...
              if($extractDataResponseResp === "Success")
              {
                ...
                if (json_last_error() === JSON_ERROR_NONE)
                {
                  ...
                  if(isset($extractDataResponseMesgReturn["param_01"]) && isset($extractDataResponseMesgReturn["param_02"]) && isset($extractDataResponseMesgReturn["param_03"]))
                  {
                    ...
                    $param_03Value = $extractDataResponseMesgReturn["param_03"];
                  ...
                  else if(isset($extractDataResponseMesgReturn["param_01"]) && isset($extractDataResponseMesgReturn["param_02"]) && isset($extractDataResponseMesgReturn["param_03"]))
                  {
                    // Set values
                    $param_01 = $extractDataResponseMesgReturn["param_01"];
                    $param_02 = $extractDataResponseMesgReturn["param_02"];
                    $param_03 = $extractDataResponseMesgReturn["param_03"];
                  }
                  else
                  {
                    // Set variables
                    $param_02 = "Check JSON Response Missing Parameters Issue";

                    // Append error message
                    array_push($errormessagearray, array('Check JSON Response Missing Parameters Issue', $value01Value, $value02Value, $value03Value, 'Error', str_replace($lineBreakString, '', $extractDataResponseMesg)));
                  }
                }
                ...
                else if (!preg_match($methodNotPermitted, $extractDataResponseMesg))
                {
                  ...
                  // Set variables
                  $param_02 = "API GET or POST method not Permitted for API call";
                }
                else
                {
                  // Set variables
                  $param_02 = "Check JSON Decode Issue";

                  // Append error message
                  array_push($errormessagearray, array('Check JSON Decode Issue', $value01Value, $value02Value, $value03Value, 'Error', str_replace($lineBreakString, '', $extractDataResponseMesg)));
                }
              }
              else
              {
                ...
                array_push($errormessagearray, array('Check Web Service CURL Call', $value01Value, $value02Value, $value03Value, 'Error', str_replace($lineBreakString, '', $param_02)));
              }
            }
            else
            {
    ...
    if (count($errormessagearray) > 0)
    {
      ...
      $colHeaderArray = array(array('Process', 'Value 01', 'Value 02', 'Value 03', 'Response', 'Message'));
    ...
    echo json_encode(array('param_01' => $param_01, 'param_02' => $param_02, 'param_03' => $param_03, 'param_04' => $param_04));
  }
  catch(Exception $e)
  {
    ...
    // Encode and send JSON back to Client-Side
    echo json_encode(array('param_01' => '0', 'param_02' => 'There was an issue with check relay', 'param_03' => '', 'param_04' => ''));
  }
?>