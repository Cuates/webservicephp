<?php
  /*
          File: web_service_allocate_relay.php
       Created: 07/23/2020
       Updated: 07/26/2020
    Programmer: Cuates
    Updated By: Cuates
       Purpose: Allocate web service Relay
  */
  ...
  $scriptName = 'Web Service Allocate Relay'; // Production
  // $scriptName = 'TEST Web Service Allocate Relay TEST'; // Development
  ...
  try
  {
    ...
    // $errorPrefixFilename = "web_service_allocate_relay_issue_"; // Production
    $errorPrefixFilename = "web_service_allocate_relay_dev_issue_"; // Development
        ...
        if(!isset($webserviceResponse['SError']) && !array_key_exists('SError', $webserviceResponse))
        {
          ...
          if ($server_response === "Success")
          {
            ...
            if (($value01Value !== "") && ($value02Value !== ""))
            {
              ...
              $extractDataResponse = $web_service_cl->allocateWebService($value01Value, $value02Value);
              ...
              if($extractDataResponseResp === "Success")
              {
                ...
                if (json_last_error() === JSON_ERROR_NONE)
                {
                  ...
                  if(isset($extractDataResponseMesgReturn["param_01"]) && isset($extractDataResponseMesgReturn["param_02"]))
                  {
                    ...
                    else if (trim($param_01Value) === -1 && strtolower($value03Value) === "yes" && preg_match("/Internal Server Error/i", $param_02Value))
                    {
                      // Set values
                      $param_01 = 5;
                      $param_02 = 'Programmer value03 2020-01-01';
                    }
                  ...
                  {
                    // Set variables
                    $param_02 = "Allocate JSON Response Missing Parameters Issue";

                    // Append error message
                    array_push($errormessagearray, array('Allocate JSON Response Missing Parameters Issue', $value01Value, $value02Value, $value03Value, 'Error', str_replace($lineBreakString, '', $extractDataResponseMesg)));
                  }
                }
                ...
                {
                  // Set variables
                  $param_02 = "Allocate JSON Decode Issue";

                  // Append error message
                  array_push($errormessagearray, array('Allocate JSON Decode Issue', $value01Value, $value02Value, $value03Value, 'Error', str_replace($lineBreakString, '', $extractDataResponseMesg)));
                }
              }
              else
              {
                ...
                array_push($errormessagearray, array('Allocate Web Service CURL Call', $value01Value, $value02Value, $value03Value, 'Error', str_replace($lineBreakString, '', $param_02)));
              }
            }
            else
            {
  ...
  catch(Exception $e)
  {
    ...
    // Encode and send JSON back to Client-Side
    echo json_encode(array('param_01' => '0', 'param_02' => 'There was an issue with allocate relay', 'param_03' => ''));
  }
?>